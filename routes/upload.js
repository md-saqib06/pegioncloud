...
router.post('/', (req, res, error) => {
    ...
    upload.array('uploadFile', 10)(req, res, async (err) => {
        try {
            ...
            const zipFileName = `pegioncloud-${shortURL}.tar.gz`;
            const zipFileDir = `./uploads/${shortURL}/${zipFileName}`;
            const output = fs.createWriteStream(zipFileDir);
            ...
            console.log("Error: Unable to process files!");
            res.status(500).send({
                error: {
                    message: "Uploading Error! File undefined..."
                }
            }).end();
        }
    });
